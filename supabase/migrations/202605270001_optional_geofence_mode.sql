...
begin
  ...
  if v_assigned_site_id is not null then
    select
      s.id,
  ...
  if v_require_geofence then
    if v_site_id is null then
      raise exception 'Geofence is required but no active site is assigned to your profile';
    end if;

    if v_site_latitude is null or v_site_longitude is null then
      raise exception 'Geofence is required but the assigned site has no coordinates configured';
    end if;
  elsif v_site_id is null then
    select
      s.id,
      s.latitude::double precision,
      s.longitude::double precision,
